...
int main(int argc, char *argv[]) {
	...
	*(dev_name + strlen(dev_name) - 1) = '\0'; //the pcap_open_live don't take the last \n in the end
	...
	// The FILTER has to be provided in arguments
	//port 53 : DNS requests filter
	if (argc >= 2) { // Case where filter

		struct bpf_program fp;		/* The compiled filter expression */
		char filter_exp[50];	/* The filter expression */
		char *filter_pointer = filter_exp;
